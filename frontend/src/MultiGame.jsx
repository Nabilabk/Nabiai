...
export default function MultiGame() {
    ...
    const updatePlayer = (updates) => {
        setPlayers(prev => prev.map(p => p.id === currentPlayer.id ? { ...p, ...updates } : p))
    }
    ...
    const handleResult = ({ emotion, confidence }) => {
        ...
        if (match) {
            if (confidence >= 0.60) {
                pts = 100;
                key = 'perfect';
                newStreak = currentPlayer.streak + 1;
        ...
        updatePlayer({ roundScore: Math.round(currentAverage), streak: newStreak })

        if (newStreak > currentPlayer.bestStreak) {
            updatePlayer({ bestStreak: newStreak })
        }
        ...
        if (currentIndex < currentTargets.length - 1) {
            ...
            if (finalScore >= currentLevel.targetScore && currentPlayer.level < LEVELS.length) {
                const nextLevel = currentPlayer.level + 1;
                    ...
                    <div className="bg-gray-800 rounded-2xl p-6 mb-8">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-left">
                            <div className="bg-gray-700 p-4 rounded-xl">
                                ...
                                <p className="text-3xl font-bold text-gold">{winner.totalScore}</p>
                            </div>
                            <div className="bg-gray-700 p-4 rounded-xl">
                                <p className="text-gray-400 text-sm">Rounds Played</p>
                                <p className="text-3xl font-bold text-white">{winner.totalRounds}</p>
                            </div>
                            <div className="bg-gray-700 p-4 rounded-xl">
                                <p className="text-gray-400 text-sm">Best Streak</p>
                                <p className="text-3xl font-bold text-purple-400">{winner.bestStreak}</p>
                            </div>
                            <div className="bg-gray-700 p-4 rounded-xl">
    ...
    return (
        ...
        >
                ...
                {showLevelComplete && (
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                Next: Level {currentPlayer.level + 1} - {LEVELS[currentPlayer.level]?.name || 'Final'} 🚀
                            </motion.p>
                        </motion.div>
                    </motion.div>
                )}
            ...
            <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-6 mt-8">
                {players.map(player => (
                    <div key={player.id} className={`rounded-2xl p-6 ${player.id === currentPlayer.id ? 'border-4 border-gold' : 'border border-gray-600'}`} style={{
                        background: 'rgba(0, 0, 0, 0.4)',
                        backdropFilter: 'blur(20px)'
                    }}>
                        <h2 className="text-3xl font-bold text-white mb-4">{player.name}</h2>
                        <p className="text-lg text-gray-400">
                            Level {player.level} — {LEVELS[player.level - 1]?.name || 'Légende'}
                        </p>
                        <div className="flex justify-center gap-2 mb-4">
                            {LEVELS.map((_, i) => (
                                <div
                                    ...
                                    style={{
                                        background: i < player.level ? 'linear-gradient(90deg, #FFD700 0%, #FFA500 100%)' : 'rgba(255, 255, 255, 0.1)'
                                    }}
                                />
                            ))}
                        </div>
                        ...
                        <div className="space-y-2 pt-4 border-t border-gray-800">
                            <div className="flex justify-between text-sm">
                                ...
                                <span className="text-white font-bold">{player.round}</span>
                            </div>
                            <div className="flex justify-between text-sm">
